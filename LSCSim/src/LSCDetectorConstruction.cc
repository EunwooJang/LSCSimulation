...
G4VPhysicalVolume * LSCDetectorConstruction::ConstructDetector()
{
  ...
  if (fWhichDetector != "PROTO" && fWhichDetector != "PROTO_ACRYLIC") {
  ...
  else if (fWhichDetector =="PROTO_ACRYLIC")
    ConstructDetector_Prototype_AcrylicBall(WorldPhys, pmtSDInner, geom_db);
