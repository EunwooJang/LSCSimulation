    ...
    G4VPhysicalVolume * worldphys, LSCPMTSD * pmtsd, GLG4param & geom_db)
{
  ...
  auto BufferTankLog = new G4LogicalVolume(BufferTankTubs, G4Material::GetMaterial("Steel_unpolished"), "BufferTankLog");
  auto visatt = new G4VisAttributes(G4Color(1, 1, 1, 0.5));
  visatt->SetForceSolid(true);
  BufferTankLog->SetVisAttributes(visatt);
  auto BufferTankPhys = new G4PVPlacement(0, G4ThreeVector(), BufferTankLog, "BufferTankPhys", WorldLog, false, 0, fGeomCheck);


  auto BufferLiquidTubs = new G4Tubs("BufferLiquidTubs", 0, bufferR - bufferT, bufferH / 2 - bufferT, 0, 360 * deg);
  auto BufferLiquidLog = new G4LogicalVolume(BufferLiquidTubs, G4Material::GetMaterial("Water"), "BufferLog");
  BufferLiquidLog->SetVisAttributes(G4VisAttributes::GetInvisible());
  auto BufferLiquidPhys = new G4PVPlacement(0, G4ThreeVector(), BufferLiquidLog, "BufferLiquidPhys", BufferTankLog, false, 0, fGeomCheck);
  ...
  auto TargetTankLog = new G4LogicalVolume(TargetTankTubs, G4Material::GetMaterial("Acrylic"), "TargetTankLog");
  visatt = new G4VisAttributes();
  visatt->SetForceSolid(true);
  TargetTankLog->SetVisAttributes(visatt);
  auto TargetTankPhys = new G4PVPlacement(0, G4ThreeVector(), TargetTankLog, "TargetTankPhys", BufferLiquidLog, false, 0, fGeomCheck);


  auto TargetLSTubs = new G4Tubs("TargetLSTubs", 0, targetR - targetT, targetH / 2 - targetT, 0, 360 * deg);
  auto TargetLSLog = new G4LogicalVolume(TargetLSTubs, G4Material::GetMaterial("LS_LAB"), "TargetLSLog");
